...
with hand crafted latent space priors: Dcc + Da + Dc
'''
...
parser = argparse.ArgumentParser(description="Variational Information Bottleneck for Semi-supervised Classification with hand crafted latent space priors: Dcc + Da + Dc")
...
parser.add_argument("--checkpoint_dir", default="Dcc_Da_Dc/Dcc_Dz_Dc_%sbn_%s_alpha_a_%s_alpha_c_%s_%snoise_%0.2f_run_%s", type=str, help="Dir where to store models")

parser.add_argument("--supervised_n", default=100,    type=int, help="The amount of supervised data")
parser.add_argument("--alpha_a",      default=0.0005, type=float, help="The regularization parameter for Da")
parser.add_argument("--alpha_c",      default=0.005,  type=float, help="The regularization parameter for Dc")
...
if __name__ == "__main__":
    ...
    for run in range(1, args.runs+1):
        ...
        info = {"checkpoint_dir_": checkpoint_dir_,
                ...
                '#_classes': args.classes, "conv_filters": args.conv_filters,
                "is_bn": args.is_bn, "alpha_a": args.alpha_a, "alpha_c": args.alpha_c,
        ...
        if not args.is_supervised_noise and args.n_noise_realisations > 0:
            train_data_unlabel = loadTrainDataWithNoise(batch_size,
                                                        data_path_="./data",
                                                        noise_std=args.noise_std,
                                                        n_noise_realisations=args.n_noise_realisations)
        else:
            train_data_unlabel = loadTrainData(batch_size, data_path_="./data")

        if args.supervised_n > 0:
            if args.is_supervised_noise and args.n_noise_realisations > 0:
                train_data_label = loadTrainDataWithNoise(batch_size,
                                                          data_path_="./data",
                                                          noise_std=args.noise_std,
                                                          n_noise_realisations=args.n_noise_realisations,
                                                          n_use_sample=args.supervised_n)
            else:
        ...
        # 3.
        Classifier_model = Model([supervised_x, unsupervised_x],
                                 [Decoder(Encoder(supervised_x)),
        ...
        Classifier_model.compile(loss=[keras.losses.categorical_crossentropy,
                                       ...
                                       keras.losses.binary_crossentropy],
                      optimizer=keras.optimizers.Adam(learning_rate=args.lr, decay=args.weight_decay),
                      metrics=['accuracy'], loss_weights=[1, args.alpha_a, args.alpha_c])
        ...
        for epoch in range(1, args.epochs+1):
            ...
            while cur_epoch == train_data_unlabel.epochs_completed:
                ...
                # 2.
                x = np.concatenate(
                    (np.tile(np.eye(10, dtype=int), (args.batch_size // 10, 1)), Decoder.predict(Encoder.predict(x_unlabeled))))
                # real images label is 1.0
                y = np.ones([2 * batch_size, 1])
                # fake images label is 0.0
                y[batch_size:, :] = 0.0
                ...
                loss = Classifier_model.train_on_batch([x_labeled, x_unlabeled], [labels,
