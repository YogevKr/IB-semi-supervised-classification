...
with learnable latent space priors: Dcc + Dz + Dc + Dxx + Dx
'''
...
parser = argparse.ArgumentParser(description="Variational Information Bottleneck for Semi-supervised Classification with learnable latent space priors: Dcc + Dz + Dc + Dxx + Dx")
...
parser.add_argument("--checkpoint_dir", default="Dcc_Dz_Dc_Dxx_Dx/Dcc_Dz_Dc_Dxx_Dx_%sbn_%s_alpha_z_%s_alpha_c_%s_alpha_xx_%s_alpha_x_%s_run_%s", type=str, help="Dir where to same models")
...
parser.add_argument("--alpha_x",     default=0, type=float, help="The regularization parameter for Dx")
...
if __name__ == "__main__":
    ...
    for run in range(1, args.runs+1):
        ...
        info = {"checkpoint_dir_": checkpoint_dir_,
                ...
                "is_bn": args.is_bn, "alpha_z": args.alpha_z, "alpha_c": args.alpha_c, "alpha_xx": args.alpha_xx, "alpha_x": args.alpha_x,
        ...
        plot_model(Dx, to_file="%s/model_Dx_scheme.png" % results_dir_, show_shapes=True)
        ...
        for epoch in range(1, args.epochs+1):
            ...
            while cur_epoch == train_data_unlabel.epochs_completed:
                ...
                input_c = np.tile(np.eye(10, dtype=int), (args.batch_size // 10, 1))
                input_z = np.random.normal(0, 1, size=[args.batch_size, args.dim_z])

                # --- Dx
                x_generated = Reconstructor.predict(
                    np.concatenate([input_z, input_c], axis=1))
                x = np.concatenate([x_unlabeled, x_generated])

                # real images label is 1.0
                y = np.ones([2 * batch_size, 1])
                # fake images label is 0.0
                y[batch_size:, :] = 0.0
                loss_dx = Dx_model.train_on_batch(x, y)
                Loss_Dx.append(loss_dx)

                # --- MSE ---------------------------
                loss = MSE_model.train_on_batch([x_unlabeled, input_z, input_c], [x_unlabeled, np.ones([batch_size, 1])])
